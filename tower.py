    ...
    def attack(self, enemy_group):
        ...
        for en in enemy_group.get():
            ...
            if distance <= self.range[self.level]:
                ...
                self.cd_count = 0
                # return：此處不能有 return，才能同時攻擊 enemy，否則會依序一個一個攻擊
...
class TowerGroup:
    ...
    def draw(self, win):
        ...
        # draw tower range：若 tower 不存在，即不會有 range & menu
    ...
    def upgrade(self):
        ...
        #
        # 判斷是哪個 tower 被點，是為 tw
        for tw in self.__towers:
            if self.selected_tower == tw:
                # upgrade 被點擊時，level + 1，不得超過 5
                if tw.level < 5:
                    tw.level += 1
    ...
    def sell(self):
        ...
        for tw in self.__towers:
            if self.selected_tower == tw:
                ...
                # 將 selected_tower 歸零，即不會 draw 出 menu 及 range (88行)
                self.selected_tower = None
